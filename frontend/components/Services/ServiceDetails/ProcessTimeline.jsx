        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-6 relative">
          <div className="hidden md:block absolute top-12 left-[12%] w-[76%] h-[2px] bg-gray-200 z-0 rounded-full">
            <motion.div
              ...
              viewport={{ once: true }}
              className="h-full bg-gradient-to-r from-[#E8622A] via-[#F28C5E] to-[#2A4A52] rounded-full relative"
            >
              <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-[#2A4A52] rounded-full shadow-[0_0_10px_#2A4A52]"></div>
            </motion.div>
          </div>
          ...
          {processes.map((process, idx) => (
            ...
            >
              <div className="w-24 h-24 bg-white border-[6px] border-[#F5F7F8] shadow-xl text-[#E8622A] rounded-full flex items-center justify-center mb-6 group-hover:-translate-y-2 group-hover:shadow-2xl transition-all duration-300">
                {process.icon}
              </div>
              <h4 className="text-xl font-bold text-[#1A2E33] mb-3">
                {process.title}
              </h4>
