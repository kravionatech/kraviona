  ...
  return <Frame>
    <main className="min-h-full bg-[#f4f8f7] px-4 py-6 sm:px-6 lg:px-9 lg:py-8">
      <div className="mx-auto max-w-[1600px]">
        <section className="relative mb-6 overflow-hidden rounded-[28px] border border-[#0f5960]/20 bg-gradient-to-br from-[#0b363d] via-[#0f5960] to-[#1a6870] p-6 text-white shadow-2xl shadow-[#0f5960]/20 sm:p-8">
          <div className="absolute -right-20 -top-16 h-72 w-72 rounded-full bg-[#f7c56d]/15 blur-3xl"/><div className="absolute bottom-0 left-1/3 h-px w-1/2 bg-gradient-to-r from-transparent via-white/35 to-transparent"/>
          <div className="relative flex flex-col justify-between gap-7 xl:flex-row xl:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-[#f7d994]"><Sparkles size={13} /> {analytics?.scope === "workspace" ? "Executive command center" : "Personal content command center"}</div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{scopeLabel}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#dcebea] sm:text-base">A secure, date-wise view of content growth, SEO quality and CMS activity. Every result is scoped to your access level.</p><div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold text-[#e5f2f1]">Role-safe reporting</span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold text-[#e5f2f1]">Live audit stream</span>{analytics?.system && <span className="rounded-full border border-emerald-300/25 bg-emerald-300/10 px-3 py-1.5 text-xs font-semibold text-emerald-100">System {analytics.system.database}</span>}</div></div>
            <div className="flex max-w-xl flex-wrap items-center gap-3 rounded-2xl border border-white/15 bg-slate-950/10 p-3 backdrop-blur-sm"><label className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-[#123f46] px-3 py-2 text-sm text-white shadow-inner shadow-black/10"><CalendarDays className="shrink-0 text-[#f7d994]" size={16} /><span className="relative"><select aria-label="Analytics date range" value={period} onChange={(event) => setPeriod(event.target.value)} className="appearance-none bg-[#123f46] py-0.5 pr-7 text-sm font-bold text-white outline-none"><option className="bg-[#123f46] text-white" value="today">Today</option>{periods.slice(1).map(([value, label]) => <option className="bg-[#123f46] text-white" key={value} value={value}>{label}</option>)}<option className="bg-[#123f46] text-white" value="custom">Custom range</option></select><ChevronDown className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 text-[#f7d994]" size={15}/></span></label>{period === "custom" && <><input aria-label="Custom range start date" type="date" value={customStart} max={customEnd} onChange={(event) => setCustomStart(event.target.value)} className="rounded-xl border border-white/15 bg-[#123f46] px-3 py-2 text-sm text-white outline-none [color-scheme:dark]"/><input aria-label="Custom range end date" type="date" value={customEnd} min={customStart} max={new Date().toISOString().slice(0, 10)} onChange={(event) => setCustomEnd(event.target.value)} className="rounded-xl border border-white/15 bg-[#123f46] px-3 py-2 text-sm text-white outline-none [color-scheme:dark]"/></>}<button type="button" onClick={loadAnalytics} disabled={loading} className="inline-flex items-center gap-2 rounded-xl bg-[#f7c56d] px-4 py-2.5 text-sm font-bold text-[#123f46] shadow-lg shadow-[#f7c56d]/15 transition hover:bg-[#ffe09c] disabled:opacity-60"><RefreshCw size={16} className={loading ? "animate-spin" : ""} /> Refresh</button></div>
          </div>
        </section>
